...
def prune_attn_uniform(k_proj, q_proj, v_proj, out_proj, sample_size):
    ...
    new_k_proj = np.zeros((heads,sample_size,k_proj.shape[-1]))
    new_q_proj = np.zeros((heads,sample_size,q_proj.shape[-1]))
    new_v_proj = np.zeros((heads,sample_size,v_proj.shape[-1]))
    new_out_proj = np.zeros((heads,sample_size,out_proj.shape[-1]))
