...
def prune_attn_randmatmul(k_proj, q_proj, v_proj, out_proj, sample_size, with_replacement, with_scaling):
    ...
    for head in range(heads):
        new_k_proj[head], new_q_proj[head], indices = sample_rows(k_proj[head], q_proj[head], sample_size, with_replacement, with_scaling)
        print(f"Head {head}: sampling indices {indices}, for QK", file=sys.stderr)

        new_v_proj[head], new_out_proj[head], indices = sample_rows(v_proj[head], out_proj[head], sample_size, with_replacement, with_scaling)
        print(f"Head {head}: sampling indices {indices}, for VO", file=sys.stderr)

    return new_k_proj, new_q_proj, new_v_proj, new_out_proj

def sample_rows(matrix1, matrix2, sample_size, with_replacement, with_scaling):
    ...
    norms2 = torch.norm(matrix2, dim=1)
    ...
    indices = np.random.choice(matrix1.shape[0], size=sample_size, p=probs, replace=with_replacement)
